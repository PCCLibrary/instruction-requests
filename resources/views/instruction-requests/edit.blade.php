    ...
    <form action="{{ route('instructionRequests.update', $instructionRequest->id) }}"
          ...
          x-data="{
                ...
                toggleEdit() {
                    ...
                    document.querySelectorAll('.edit-field input, .edit-field select, .edit-field textarea')
                        .forEach(el => el.disabled = !this.isEditing);
                },
                validateForm() {
        ...
        <div class="grid grid-cols-12 gap-6 items-start">
            ...
            <div class="col-span-12 md:col-span-8">
                ...
                <x-card title="Contact Information" class="bg-white mb-4">
                    @include('instruction-requests.partials.contact-info')
                </x-card>

                {{-- Request Information Card --}}
                @include('instruction-requests.partials.request-info')

                {{-- Date and Time Fields --}}
                @include('instruction-requests.partials.date-time-fields')

                {{-- ADA Provisions --}}
                @include('instruction-requests.partials.ada-provisions')

                {{-- Learning Outcomes --}}
                @if($instructionRequest->instruction_type !== 'asynchronous')
                    @include('instruction-requests.partials.learning-outcomes')
                @endif

                {{-- Instruction Goals --}}
                @include('instruction-requests.partials.instruction-goals')
